...
# --- TAB 1: PRODUCTION (With Special Notes) ---
with t_prod:
    with st.form(key=f"f_{st.session_state.hub}", clear_on_submit=True):
        ...
        notes = st.text_area("🗒️ Special Production Notes (Instructions/Issues)")
    ...
    if all_logs:
        ...
        if not df_live.empty:
            ...
            st.dataframe(df_live[['unit_no', 'job_code', 'part_name', 'request_date', 'required_date', 'Days Left', 'priority', 'special_notes']], use_container_width=True, hide_index=True)

# --- TAB 2: INCHARGE ---
with t_inch:
    pending = [j for j in all_logs if j['status'] != "Finished"]
    for j in pending:
        with st.expander(f"Unit {j['unit_no']} | {j['job_code']} | Notes: {j.get('special_notes', 'None')}"):
            ...
            if st.button("🚀 Start Production", key=f"s{j['id']}", use_container_width=True):
                upd = {"status": "In-House" if m=="Own" else "Outsourced", "machine_id": r_sel, "operator_id": w_sel if m=="Own" else None, "vendor_id": w_sel if m=="Contractor" else None}
                conn.table(DB_TABLE).update(upd).eq("id", j['id']).execute(); st.rerun()
            if j['status'] in ["In-House", "Outsourced"] and st.button("🏁 Finish Job", key=f"f{j['id']}", use_container_width=True):
...
with t_exec:
    if all_logs:
        ...
        s1.metric("Total Load", len(df_e)); s2.metric("WIP", len(df_e[df_e['status'] != 'Finished'])); s3.metric("Completed", len(df_e[df_e['status'] == 'Finished']))
        ...
        if 'required_date' in df_e.columns:
            ...
            if not lagging.empty:
                st.dataframe(lagging[['job_code', 'request_date', 'required_date', 'Lag', 'special_notes']], use_container_width=True, hide_index=True)
            else: st.success("✅ No lagging jobs.")
