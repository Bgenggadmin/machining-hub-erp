...
with t_prod:
    with st.form(key=f"f_{st.session_state.hub}", clear_on_submit=True):
        ...
        notes = st.text_area("🗒️ Special Production Notes") # ADDED NOTES FIELD
        ...
        if st.form_submit_button("Submit"):
            if j_code:
                conn.table(DB_TABLE).insert({
                    ...
                    "special_notes": notes # ADDED NOTES TO INSERT
                }).execute()
                st.rerun()
    ...
    if all_logs:
        ...
        if not df_live.empty and 'required_date' in df_live.columns:
            ...
            # --- MANDATORY DISPLAY COLUMNS ---
            cols_to_show = ['unit_no', 'job_code', 'part_name', 'request_date', 'required_date', 'Days Left', 'priority', 'status', 'special_notes']
            st.dataframe(df_live[cols_to_show], use_container_width=True, hide_index=True)
        else: st.info("No active jobs or check database columns.")

# --- TAB 2: INCHARGE DESK ---
with t_inch:
    pending = [j for j in all_logs if j['status'] != "Finished"]
    for j in pending:
        with st.expander(f"Unit {j['unit_no']} | {j['job_code']} | Status: {j['status']}"):
            st.info(f"📝 Notes: {j.get('special_notes', 'No special instructions')}")
    ...
    if all_logs:
        ...
        if 'required_date' in df_e.columns:
            ...
            if not lagging.empty:
                st.dataframe(lagging[['job_code', 'request_date', 'required_date', 'Lag', 'priority', 'special_notes']], use_container_width=True, hide_index=True)
            else:
                st.success("✅ All jobs are on track.")
