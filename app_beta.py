...
# --- DYNAMIC CONFIG (The "Engine" of the App) ---
...
all_logs, resource_list, operator_list, vendor_list = fetch_all()
...
with t_inch:
    ...
    for j in pending:
        with st.expander(f"Unit {j['unit_no']} | {j['job_code']} | {j['status']}"):
            ...
            if st.button("Start", key=f"s{j['id']}"):
                conn.table(DB_TABLE).update({"status": "In-House" if m=="Own" else "Outsourced", "machine_id": r_sel, "operator_id": w_sel if m=="Own" else None, "vendor_id": w_sel if m=="Contractor" else None}).eq("id", j['id']).execute()
                st.rerun()
            if j['status'] in ["In-House", "Outsourced"]:
                if st.button("Finish", key=f"f{j['id']}"):
...
with t_exec:
    if all_logs:
        ...
        c2.metric("Pending", len(df_e[df_e['status'] != 'Finished']))
        c3.metric("Finished", len(df_e[df_e['status'] == 'Finished']))
...
with t_masters:
    ...
    with col2:
        ...
        if st.button("Add Staff"):
            conn.table("beta_operator_master" if m_type=="Operator" else "beta_vendor_master").insert({"operator_name" if m_type=="Operator" else "vendor_name": m_name}).execute()
            st.rerun()
