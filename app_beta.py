...
def get_all_data():
    try:
        ...
        o_data = conn.table("beta_operator_master").select("operator_name").execute().data or []
        v_data = conn.table("beta_vendor_master").select("vendor_name").execute().data or []
        vh_data = conn.table("beta_vehicle_master").select("vehicle_number").execute().data or []
        
        # Fetch Logs
        logs = conn.table(DB_TABLE).select("*").order("created_at", desc=True).execute().data or []
        
        # Convert to DataFrame and SANITIZE (Ensure all required columns exist)
        ...
        for col in required_cols:
            if col not in df.columns:
                df[col] = None # Create missing column with Nulls so code doesn't break
...
# --- TAB 2: INCHARGE ENTRY DESK (LOGISTICS RESTORED) ---
with tabs[1]:
            ...
            with st.expander(f"📌 {job['job_code']} - {job['part_name']} ({job['status']})"):
                ...
                if job['status'] == "Pending":
                    ...
                    if mode == "In-House":
                        ...
                        if st.button("🚀 Start In-House", key=f"b_ih_{job['id']}"):
                        ...
                        if st.button("🚚 Dispatch to Vendor", key=f"b_os_{job['id']}"):
                    ...
                    if st.button("✅ Mark Received", key=f"b_rc_{job['id']}"):
                    ...
                    if st.button("🏁 Mark Finished", key=f"b_fi_{job['id']}"):
                        conn.table(DB_TABLE).update({"status": "Finished", "delay_reason": d_r, "intervention_note": i_n}).eq("id", job['id']).execute(); st.rerun()
    else:
        st.warning("Database is empty or connection lost.")

# --- TAB 3: EXECUTIVE ANALYTICS (SUMMARY) ---
with tabs[2]:
    if not df_main.empty:
